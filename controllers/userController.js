...
export const deleteUserAccount = async (req, res) => {
  try {
    ...
    if (user.personalPicture) {
      const personalPath = path.join(process.cwd(), 'uploads', user.personalPicture);
...
export const getUserPublicProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select('username email location role createdAt personalPicture');
